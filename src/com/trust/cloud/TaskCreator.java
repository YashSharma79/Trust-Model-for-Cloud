...
public class TaskCreator {
	...
	String[] conf = {};
		...
		fw.close();
                try {
                    File confFile = new File("/Applications/MAMP/htdocs/test/cloud.txt");
                    FileReader fileReader  = new FileReader(confFile);
                    BufferedReader reader = new BufferedReader (fileReader);
                    System.out.println("Attempting to read from file in: "+confFile.getCanonicalPath());
		...
		try {
			...
			//VM description
			int vmid = 0;
			long size = Integer.parseInt(conf[8]); //image size (MB)
			int ram = Integer.parseInt(conf[9]);//vm memory (MB)
                        int mips = Integer.parseInt(conf[10]);
			long bw =  Long.parseLong(conf[11]);
			int pesNumber = 1; //number of cpus
			String vmm = "Xen"; //VMM name
                        System.out.println("VM:"+mips+" "+size+" "+" "+ram);
	...
	private static void printCloudletList(List<Cloudlet> list) throws IOException {
		...
		for (int i = 0; i < size; i++) {
			...
			if (Cloudlet.SUCCESS == cloudlet.getCloudletStatus()) {
				Log.print("SUCCESS");
				...
				Log.printLine(indent + indent + cloudlet.getResourceId()
						...
						+ dft.format(cloudlet.getFinishTime()));
                                FileWriter fw = new FileWriter("cr7.txt");
                                fw.write(cloudlet.getExecStartTime()+" "+cloudlet.getFinishTime());
                                fw.close();
